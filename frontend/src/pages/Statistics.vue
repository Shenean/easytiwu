  ...
  <n-card title="题库统计" class="stats-card">
    <n-space vertical size="large">
      ...
      <n-spin :show="loading" description="正在加载统计数据...">
        ...
        <!-- 空状态 -->
        <n-empty v-else-if="!loading && !stats" description="暂无统计数据" />
      </n-spin>
...
const tableData = computed<StatisticsTableRow[]>(() => {
  ...
  return Object.entries(stats.value.byType).map(([type, v]) => ({
    type,
    count: v.count ?? 0,
...
const fetchStatistics = async () => {
  try {
    ...
    if (response.data && typeof response.data === 'object') {
      const apiResponse = response.data as ApiResponse<StatisticsOverview>
      
      if (apiResponse.success && apiResponse.data) {
        stats.value = apiResponse.data
        console.log('统计数据加载成功:', stats.value)
      } else {
        throw new Error(apiResponse.message || '获取统计数据失败')
      }
    } else {
    ...
    if (err.response) {
      ...
      switch (status) {
        case 404:
          ...
          break
        case 500:
          error.value = `服务器内部错误: ${message}`
          break
        case 503:
          error.value = '统计服务暂时不可用，请稍后重试'
          break
        default:
          error.value = `请求失败 (${status}): ${message}`
      }
