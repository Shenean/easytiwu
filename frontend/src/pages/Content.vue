  ...
  <div class="page-container">
    ...
    <div v-else-if="questionList.length === 0" class="empty-container">
      <n-empty description="暂无题目数据">
        <template #extra>
          <BaseButton @click="$router.go(-1)">返回上一页</BaseButton>
        </template>
      </n-empty>
    </div>
    ...
    <template v-else>
      ...
      <QuestionActions
        ...
        :has-next="currentQuestionIndex < questionList.length - 1"
...
import { contentAPI } from '../api/config' // ← 与你提供的 config.ts 匹配（命名导入）
...
async function fetchQuestions() {
  try {
    ...
    if (!contentAPI || typeof (contentAPI as any).getQuestions !== 'function') {
...
async function submitAnswer() {
  ...
  try {
    ...
    if (!contentAPI || typeof (contentAPI as any).verifyAnswer !== 'function') {
